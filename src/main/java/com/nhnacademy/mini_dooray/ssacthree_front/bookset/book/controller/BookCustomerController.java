...
public class BookCustomerController {
    ...
    public String getBooksByFilters(
        ...
        // 데이터 가져오기
        Page<BookListResponse> books = getBooksByFilter(page, size, sort, authorId, categoryId,
            tagId);
        ...
        model.addAttribute("extraParams",
            extraParams.isEmpty() ? "" : "&" + extraParams); // 항상 &로 시작
    ...
    public String showHomeBook(
        ...
        Page<BookListResponse> banner2Page = bookCommonService.getAllAvailableBooks(page, 1,
            viewSort);
        BookListResponse banner2 =
            banner2Page.getContent().isEmpty() ? null : banner2Page.getContent().get(0);

        Page<BookListResponse> awardBooks = bookCommonService.getBooksByAuthorId(page, size, sort,
            authorId); // 한강 작가
    ...
    public String showBook(@PathVariable("book-id") Long bookId,
        ...
        Page<BookReviewResponse> reviews = reviewService.getReviewsByBookId(page, size, sort,
            bookId);
